...
void Uart_user_inst(uint8_t *cmd);
...
void usart2_EnQueue(uint8_t data)
{
	...
	if(usart2_IsFullQueue() == true){
		printf("Queue Full Error : %s on line: %d \r\n",__FILE__, __LINE__);
	}

	usart2_queue[usart2_rear] = data;
/*
	#else
	if(((usart2_rear+1) % USART2_QUEUE_SIZE) == usart2_front) queue full
	{
		printf("Queue full error :%s on line:%d \r\n", __FILE__, __LINE__);
		return;
	}
...
uint8_t usart2_DeQueue(void)
{
	...
	return usart2_queue[usart2_front];
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	if (huart->Instance == USART2)
	{
		...
		//Uart_user_inst();
		//queue
		usart2_EnQueue(usart2_rxdata);
		if(HAL_UART_Receive_IT(huart, &usart2_rxdata, 1) != HAL_OK)	
		{
			/* error handling */
		}
	}	
...
void Uart_user_inst(uint8_t *cmd){
	...
	if (strcmp((char *) cmd, "version")==0){
		printf("\r\n VERSION : %s\r\n", __VERSION__);
	}
	if (strcmp((char*) cmd, "trace") == 0) {
		printf("\r\n error : %s on line : %d\r\n", __FILE__, __LINE__); // have to move
	}
